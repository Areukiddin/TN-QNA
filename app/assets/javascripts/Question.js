class Question {
  ...
  voteLink(id, liked) {
    let link = $(
      `<a class="voting ${liked ? "upVote" : "downVote"}">${
        liked ? "Like" : "Dislike"
      }</a>`
    );
  ...
  voteEvent(event) {
    ...
    $(`#question-${vote.id} .downVote`).replaceWith(
      this.voteCancelLink(vote.id)
    );
  }
  ...
  static jsonRender(data) {
    ...
    question.append(
      `<a href="/questions/${locals.question.id}">${locals.question.title}</a>`
    );
